...
<div class="flex h-screen overflow-hidden">
    ...
    <aside class="w-60 bg-white flex flex-col flex-shrink-0 border-r border-amber-100">
        ...
        <nav class="flex-1 px-3 py-5 space-y-0.5 overflow-y-auto">
            ...
            <div x-data="{ open: {{ request()->routeIs('admin.adopters.*') || request()->routeIs('admin.volunteers.*') ? 'true' : 'false' }} }">
                <button @click="open = !open"
                        class="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition
                               {{ request()->routeIs('admin.adopters.*') || request()->routeIs('admin.volunteers.*') ? 'bg-amber-50 text-amber-700 font-semibold' : 'text-gray-400 hover:bg-amber-50 hover:text-amber-700' }}">
                    <svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" stroke-width="1.75" viewBox="0 0 24 24">
                ...
                <div x-show="open"
                    ...
                    <div x-data="{ appOpen: {{ request()->routeIs('admin.adopters.*') || request()->routeIs('admin.volunteers.*') ? 'true' : 'false' }} }">
            ...
            <a href="{{ route('admin.analytics.index') }}"
               class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition
                      {{ request()->routeIs('admin.analytics.*') ? 'bg-amber-50 text-amber-700 font-semibold' : 'text-gray-400 hover:bg-amber-50 hover:text-amber-700' }}">
                <svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" stroke-width="1.75" viewBox="0 0 24 24">
            ...
            <div x-data="{ open: {{ request()->routeIs('admin.events.*') ? 'true' : 'false' }} }">
                <button @click="open = !open"
                    ...
                    <span class="flex-1 text-left">Web Management</span>
                    <svg class="w-3 h-3 transition-transform duration-200" :class="open ? 'rotate-180' : ''" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7"/>
                    </svg>
                </button>
        ...
        <div class="px-3 pb-5 space-y-0.5 border-t border-amber-100 pt-4">
            <a href="{{ route('admin.messages.index') }}"
               class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition
                      {{ request()->routeIs('admin.messages.*') ? 'bg-amber-50 text-amber-700 font-semibold' : 'text-gray-400 hover:bg-amber-50 hover:text-amber-700' }}">
                <svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" stroke-width="1.75" viewBox="0 0 24 24">
