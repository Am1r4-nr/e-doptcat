...
<div class="mb-8 flex items-start justify-between">
    <div>
        <h1 class="text-3xl font-serif font-semibold text-gray-800">Event Management</h1>
...
<div class="grid grid-cols-4 gap-4 mb-8">
    <div class="bg-white rounded-2xl p-5 shadow-sm border border-amber-100">
        <p class="text-[10px] tracking-widest text-amber-500 uppercase font-semibold mb-2">Total Events</p>
        <p class="text-4xl font-bold text-gray-800">{{ $total }}</p>
        <p class="text-xs text-gray-400 mt-1">All time</p>
    </div>
    <div class="bg-white rounded-2xl p-5 shadow-sm border border-amber-100">
        <p class="text-[10px] tracking-widest text-amber-500 uppercase font-semibold mb-2">Upcoming</p>
        <p class="text-4xl font-bold text-amber-600">{{ $upcoming }}</p>
        <p class="text-xs text-gray-400 mt-1">Scheduled ahead</p>
    </div>
    <div class="bg-white rounded-2xl p-5 shadow-sm border border-amber-100">
        <p class="text-[10px] tracking-widest text-amber-500 uppercase font-semibold mb-2">Completed</p>
    ...
    <div class="bg-amber-600 rounded-2xl p-5 shadow-sm">
        <p class="text-[10px] tracking-widest text-amber-200 uppercase font-semibold mb-2">Cancelled</p>
...
<div class="bg-white rounded-2xl shadow-sm border border-amber-100 overflow-hidden">
    <div class="overflow-x-auto">
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-gray-50">
                        ...
                        <td class="py-4 px-6">
                            <div class="flex items-center justify-end gap-3">
                                <a href="{{ route('admin.events.edit', $event) }}"
                                   class="text-xs font-semibold text-amber-600 hover:text-amber-800 transition">
                                    Edit
                                </a>
                ...
                @empty
                    <tr>
                        <td colspan="6" class="py-12 text-center text-sm text-gray-400 italic font-serif">
                            No events yet. <a href="{{ route('admin.events.create') }}" class="text-amber-600 not-italic font-semibold">Create one →</a>
                        </td>
                    </tr>
                @endforelse
            </tbody>
    ...
    @if($events->hasPages())
        <div class="px-6 py-4 border-t border-amber-50 flex items-center justify-between bg-[#FAF8F5]">
            ...
            <div class="flex items-center gap-1">
                ...
                @foreach($events->getUrlRange(1, $events->lastPage()) as $page => $url)
                    @if($page == $events->currentPage())
                        <span class="w-8 h-8 flex items-center justify-center rounded-full bg-amber-600 text-white text-xs font-bold shadow">{{ $page }}</span>
                    @else
