...
<div class="max-w-6xl mx-auto">
    ...
    <div class="mb-8 flex items-center gap-4">
        <a href="{{ route('admin.cats.show', $cat) }}" class="text-gray-400 hover:text-[#C9A84C] transition text-lg">←</a>
        <div>
            <p class="text-[10px] tracking-widest text-[#C9A84C] uppercase font-semibold">Edit Record</p>
            <h1 class="text-3xl font-cabinet font-semibold text-gray-800">Update Feline Profile</h1>
                ...
                <div class="bg-white rounded-2xl border border-[#E8E2D8] shadow-sm p-7 space-y-5">
                    ...
                    <div>
                        ...
                        <textarea name="description" rows="3"
                            class="w-full px-4 py-2.5 text-sm bg-[#FAF6F0] border border-[#E8E2D8] rounded-xl focus:outline-none focus:ring-1 focus:ring-[#C9A84C] resize-none">{{ old('description', $cat->description) }}</textarea>
                    </div>
                    ...
                    <div>
                        ...
                        @if($cat->image)
                            <div class="mb-3 flex items-center gap-3">
                                <img src="{{ asset('storage/' . $cat->image) }}" class="w-16 h-16 rounded-xl object-cover border border-[#E8E2D8]">
                    ...
                    <div class="flex gap-3">
                        <button type="submit"
                            class="flex-1 py-3 bg-[#C9A84C] hover:bg-[#b8963e] text-white font-semibold rounded-full transition shadow-sm shadow-amber-200">
                            Save Changes
                        </button>
                        <a href="{{ route('admin.cats.show', $cat) }}"
                            class="flex-1 py-3 text-center bg-[#FAF6F0] hover:bg-[#FAF8F0] text-gray-600 font-semibold rounded-full border border-[#E8E2D8] transition">
                            Cancel
                        </a>
                    </div>
                </div>
            ...
            <div class="bg-gray-800 rounded-2xl overflow-hidden shadow-lg">
                <div class="relative h-52 bg-gray-700 flex items-center justify-center">
                    ...
                    <div class="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-gray-900/80 to-transparent p-4">
                        ...
                        <p class="text-xl font-cabinet font-semibold text-white" id="previewNameDisplay">{{ $cat->name }}</p>
                    </div>
                </div>
                <div class="p-4">
                    <div class="flex gap-2 mb-3">
                        ...
                        <span class="px-2.5 py-0.5 bg-[#C9A84C] text-white text-[10px] font-bold uppercase rounded-full tracking-wide">{{ $cat->status }}</span>
                    </div>
